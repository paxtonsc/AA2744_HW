...
class ParticleFilter(object):
    ...
    def transition_update(self, u, dt):
        ...
        u_sampled = np.random.multivariate_normal(u, self.R, size=self.M)
        self.xs = self.transition_model(u_sampled, dt)
...
class MonteCarloLocalization(ParticleFilter):
    ...
    def transition_model(self, us, dt):
        ...
        gr_om_ind = np.argwhere(abs(om) > EPSILON_OMEGA).squeeze()
        ls_om_ind = np.argwhere(abs(om) <= EPSILON_OMEGA).squeeze()

        gr_th_next = self.xs[gr_om_ind, 2] + dt * om[gr_om_ind]
        gr_x_update = np.divide(V[gr_om_ind], om[gr_om_ind]) * (np.sin(gr_th_next) - np.sin(self.xs[gr_om_ind, 2]))
        gr_y_update = np.divide(V[gr_om_ind], om[gr_om_ind]) * (-np.cos(gr_th_next) + np.cos(self.xs[gr_om_ind, 2]))
        ...
        if ls_om_ind.size != 0:
            g[ls_om_ind, :] = self.xs[ls_om_ind, :] + np.vstack((ls_x_update, ls_y_update, ls_th_update)).T
    ...
    def measurement_update(self, z_raw, Q_raw):
        ...
        # print(f'xs: {xs}')
        # print(f'shpe of xs: {xs.shape}')
        vs, Q = self.measurement_model(z_raw, Q_raw)
        # print(f'shpe of vs: {vs.shape}')
        # print(f'shpe of Q: {Q.shape}')
        ws = scipy.stats.multivariate_normal.pdf(vs, cov = Q)

        #ws0 = scipy.stats.multivariate_normal.pdf(xs[0,:], mean = vs[0,:], cov = Q)
        #ws1 = scipy.stats.multivariate_normal.pdf(xs[1,:], mean = vs[1,:], cov = Q)
#
        #print('ws0: {}'.format(ws0))
        #print('ws1: {}'.format(ws1))
    ...
    def compute_innovations(self, z_raw, Q_raw):
        ...
        for m in range(self.M):
            for i in range(z_raw.shape[1]):
                ...
                z_alpha = z_raw[0,i]
                diff_r = z_r - hs[m,1,:]
                diff_alpha = angle_diff(z_alpha, hs[m,0,:])
                ...
                d = v_stacked.transpose(0,2,1) @ np.linalg.inv(Q_raw[i]) @ v_stacked
                d = np.squeeze(d)
    ...
    def compute_predicted_measurements(self):
        ...
        for i in range(self.M):
            for j in range(self.map_lines.shape[1]):
                ...
                hs[i,:,j] = tb.normalize_line_parameters(np.array(h))
        ########## Code ends here ##########
